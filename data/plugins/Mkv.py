    ...
    def on_mkv_clicked(self, widget, data=None):
        ...
        def mkvmerge():
            ...
            for count, filename in enumerate(filenames):
                ...
                sproc = subprocess.Popen(args, stdout=subprocess.PIPE, env=my_env)
                sproc.stdout.readline()

                line = ""
                while sproc.poll() is None:
                    # read progress from stdout
                    char = sproc.stdout.read(1)
        ...
        def complete():
            if len(self.errors) == 0:
                ...
                self.gui.main_window.change_status(
                    0,
                    f"Erfolgreich {str(self.success)}/{str(len(filenames))} Dateien "
                    f"umgewandelt. (Fehler: {' '.join(self.errors.values())})",
                )
    ...
    def get_norm_volume(self, filename):
        ...
        try:
            process2 = subprocess.Popen(
                [otrvpath.get_tools_path("intern-sox"), "-p", "--null", "stat", "-v"],
