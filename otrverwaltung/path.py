    ...
    if os.path.expanduser("~") in os.path.abspath(sys.path[0]):
        ...
        if os.path.exists(os.path.join(BaseDirectory.xdg_data_home, "otrverwaltung/intern-VirtualDub", 'VERSION')):
            if not filecmp.cmp(getdatapath("tools/intern-VirtualDub", 'VERSION'),
                               os.path.join(BaseDirectory.xdg_data_home, "otrverwaltung/intern-VirtualDub", 'VERSION')):
                ...
                copytree(getdatapath('tools/intern-VirtualDub'),
                         os.path.join(BaseDirectory.xdg_data_home, "otrverwaltung/intern-VirtualDub"), symlinks=True)
        else:
            copytree(getdatapath('tools/intern-VirtualDub'),
                     os.path.join(BaseDirectory.xdg_data_home, "otrverwaltung/intern-VirtualDub"), symlinks=True)
