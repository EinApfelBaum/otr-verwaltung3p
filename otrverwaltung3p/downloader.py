...
class Download:
    ...
    def _parse_time(time_string):
        ...
        m = re.match(r"((?P<h>[0-9]*)h)?((?P<m>[0-9]{1,2})m)?((?P<s>[0-9]{1,2})s)?", time_string)
        if m:
            ttime = 60 * 60 * int(m.group("h")) if m.group("h") else 0
            ttime = (ttime + 60 * int(m.group("m"))) if m.group("m") else ttime
            ttime = (ttime + int(m.group("s"))) if m.group("s") else ttime
            return ttime
        else:
            return 0
    ...
    def _download(self):
        ...
        if self.information["download_type"] == DownloadTypes.TORRENT:
            ...
            torrent_filename = os.path.join(
                self._config.get("general", "folder_new_otrkeys"), self.filename + ".torrent",
            )
            ...
            while self.__process.poll() is None:
                line = self.__process.stdout.readline().strip().decode("UTF-8")

                if "Checksum" in line:
                    result = re.findall(r"Checksum:.*\((.*%)\)", line)
                    if result:
                    ...
                    result = re.findall(r"ratio:(.*)\) ", line)
                    if result:
                        self.information["ratio"] = result[0]

                    result = re.findall(r"UP:(.*)\((.*)\)", line)
                    if result:
                    ...
                    if not self.information["size"]:
                        try:
                            ...
                            size = re.findall(r"SIZE:.*/(.*)MiB\(", line)[0]
                            size = size.replace(",", "")
                    ...
                    # get speed, est
                    if "UP" in line:
                        result = re.findall(r"SPD:(.*) UP:(.*)\((.*)\) ETA:(.*)]", line)
                        if result:
            ...
            if self.information["preferred_downloader"] == "wget":
                ...
                while True:
                    ...
                    line = self.__process.stderr.readline().strip().decode("UTF-8")
                    ...
                    if line:
                        ...
                        if "%" in line:
                            result = re.findall(r"([0-9]{1,3})% (.*)[ =](.*)", line)
                ...
                # Process is terminated
                yield self.__process.stderr.read().strip().decode("UTF-8")
                ...
                if self.information["status"] == DownloadStatus.RUNNING:
                    ...
                    self.information[
                        "message_short"
                    ] = "Versuche Download zu starten ... eventuell erst Warteschlange durchlaufen ..."
                    self.update_view()
                    ...
                    while self.__process.poll() is None:
                        ...
                        if "X-OTR-Queueposition" in line or "errorCode=29" in line:
                            match = re.search(r"X-OTR-Queueposition: ([0-9]+)", line)
                            if match:
                                self.information["message_short"] = "OTR Warteschlange - %s" % match.group(0)
                                self.update_view()
                            else:
                                if "OTR" not in self.information["message_short"]:
                            ...
                            if "FileAlloc" in line:
                                result = re.findall(r"FileAlloc:.*\(([0-9]{1,3}%)", line)
                            ...
                            if not self.information["size"]:
                                try:
                                    # aria2c gives size always in MiB (hopefully)
                                    size = re.findall(r".*/(.*)\(", line.strip())[0]
                                    size = size.strip("MiB")
                                    size = size.replace(",", "")
                                    size = int(round(float(size))) * 1024 * 1024
                                    self.information["size"] = size
                            ...
                            result = re.findall(r"\(([0-9]{1,3})%\).*CN:([0-9]{1,5}).*DL:(.*) ETA:(.*)]", line,)
                ...
                if "otrkey" in self.filename:
                    ...
                    if torrent_check_return == 0:
                        ...
                        self.information["status"] = DownloadStatus.ERROR
                        self.information["message_short"] = (
                            "(INPR):Download unvollständig - Datei hat Torrent-Prüfung nicht bestanden. z.B. durch "
                            "fehlerhaften oder nicht mehr gültiger Link. Downloadabbruch oder defekte Mirrordatei."
                        )
                        self.information["progress"] = 0
                    else:
                        self.information["status"] = DownloadStatus.ERROR
                        self.information["message_short"] = (
                            "(EER):Fehler bei der abschliessenden Überprüfung aufgetreten. "
                            "Eventuell OTR nicht erreichbar."
                        )
                        self.information["progress"] = 0

        elif self.information["download_type"] in [
            DownloadTypes.OTR_DECODE,
            DownloadTypes.OTR_CUT,
        ]:
            ...
            if self.information["download_type"] == DownloadTypes.OTR_CUT:
                ...
                if not self.information["cutlist"]:
                    ...
                    error = cutlist.download(
                        self._config.get("general", "server"), os.path.join(self.information["output"], self.filename),
                    )
                    if error:
                        self.information["status"] = DownloadStatus.ERROR
            ...
            try:
                ...
                yield (
                    "Der Pfad zum Dekoder scheint nicht korrekt zu sein. Der folgende Befehl konnte nicht "
                    "ausgeführt werden\nFehlermeldung: {error}"
                )
                return
