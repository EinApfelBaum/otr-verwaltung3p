...
class CutSmartMkvmerge(Cut):
    def __init__(self, app, gui):
        ...
        if not os.path.isdir(self.config.get("smartmkvmerge", "workingdir").rstrip(os.sep)):
            ...
            self.gui.message_info_box(
                f"Das in Einstellungen:Schneiden:SmartMKVmerge angegebene "
    ...
    def cut_file_by_cutlist(self, filename, cutlist=None, program_config_value=None):
        ...
        if vformat == Format.HQ:
            ...
            return (
                None,
                "Format nicht unterstützt (Nur MP4 H264, HQ H264 und HD H264 sind möglich).",
            )
        ...
        command = [
            ...
            "-o",
            self.workingdir + "/audio_copy.mkv",
        ] + audio_import_files
        ...
        for frame_start, frames_duration in cutlist.cuts_frames:
            ...
            if result is not None:
                ...
                return (
                    None,
                    (
                        "Cutlist oder zu schneidende Datei passen nicht zusammen oder sind fehlerhaft."
            ...
            if encoder_engine == "x264":
                command = (
                    ...
                    + [
                        ...
                        "ffms",
                        "--index",
                        self.workingdir + "/x264.index",
                        "--seek",
                        str(start + x_offset[0]),
                        "--frames",
                        str(duration + x_offset[1]),
                        "--output",
                ...
                command = [
                    ...
                    "-hide_banner",
                    "-ss",
                    str(self.seconds_to_hms((start + bframe_delay) / fps)),
                    "-i",
                    filename,
                    "-vframes",
                    str(duration),
        ...
        if video_splitframes:
            command = [
                ...
                "-o",
                self.workingdir + "/video_copy.mkv",
                filename,
            ]
            self.log.debug(f"Command executed: {command}")
            try:
        ...
        if os.path.isfile(self.workingdir + "/video_copy.mkv"):
            os.rename(
                self.workingdir + "/video_copy.mkv", self.workingdir + "/video_copy-001.mkv",
            )
        ...
        if self.config.get("smartmkvmerge", "remux_to_mp4"):
            cut_video = (
                self.workingdir
                + "/"
                + os.path.basename(os.path.splitext(self.generate_filename(filename, 1))[0] + ".mkv")
            )
        else:
        ...
        try:
            ...
            return (
                None,
                "MKVMerge konnte nicht aufgerufen werden oder zu alt (6.5.0 benötigt)",
            )
        ...
        if self.config.get("smartmkvmerge", "remux_to_mp4"):
            ...
            args = [
                self.config.get_program("ffmpeg"),
                "-i",
                cut_video,
                "-map",
                "0:v",
